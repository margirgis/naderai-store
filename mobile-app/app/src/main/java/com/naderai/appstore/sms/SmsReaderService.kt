...
import com.naderai.appstore.MainActivity
import com.naderai.appstore.R

/**
 * Foreground Service لمراقبة Vodafone Cash ومزامنة مهام الشحن.
 * لا نحذف المهمة محليًا إلا بعد تأكيد نجاح إرسال النتيجة للسيرفر.
 */
class SmsReaderService : Service() {
    ...
    private fun handleIncomingSms(sender: String, body: String) {
        ...
        if (matched != null) {
            ...
            // لا نرمي الرسالة؛ نطلب مزامنة فورية لاحتمال أن الطلب وصل للسيرفر للتو.
            heartbeatManager?.refreshNow()
            Log.d(TAG, "SMS has no current task; requested immediate task refresh")
        }
    }
    ...
    private fun processPendingTasksFromInbox() {
        ...
        try {
            ...
            for (task in pendingTasks.toList()) {
                ...
                sendTaskResult(task, sms.second, success = true)
            }
        } catch (e: Exception) {
