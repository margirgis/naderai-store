...
-- ═══════════════════════════════════════════════════════════════════════
-- Migration 00071: إصلاح تحقق sender_phone في confirm_payment_order
-- المشكلة: لو payment_order.sender_phone فاضي (NULL أو '')
--          والجهاز بعت phone — المقارنة تفشل وترجع manual_review
--          وكذلك لو الجهاز بعت phone فاضي — نفس المشكلة
-- الإصلاح: نتخطى فحص phone فقط لو أحد الطرفين فاضي
-- (الدالة الكاملة موجودة في migration 00069، هذا patch للـ sender_phone guard فقط)
-- ═══════════════════════════════════════════════════════════════════════

-- نحدث STEP 6 فقط بـ patch موضعي داخل دالة مستقلة للاختبار
-- ثم نعيد بناء confirm_payment_order كاملاً مع الـ patch
...
BEGIN
  ...
  -- ── STEP 5: SMS timestamp (نافذة 60 دق قبل إنشاء الطلب) ──────────────
  IF p_sms_timestamp IS NOT NULL THEN
    IF p_sms_timestamp < (v_order.created_at - SMS_WINDOW) THEN
      ...
      UPDATE payment_orders SET
        ...
        failure_reason='SMS أقدم من النافذة المسموحة — يحتاج مراجعة يدوية'
    ...
    IF p_sms_timestamp < (now() - MAX_SMS_AGE) THEN
      ...
      UPDATE payment_orders SET
        status='manual_review',
        failure_reason='SMS منتهية الصلاحية — يحتاج مراجعة يدوية'
      WHERE id=p_order_id AND status NOT IN ('confirmed','cancelled','failed','expired','duplicate');
      RETURN jsonb_build_object('ok',false,'scan_status','manual_review','reason','sms_expired');
    END IF;
  END IF;

  -- ── STEP 6: Sender phone (الإصلاح: تخطى لو أحد الطرفين فاضي) ─────────
  ...
  -- نتحقق فقط لو كلا الطرفين فيهما رقم — لو أحدهما فاضي نتخطى
