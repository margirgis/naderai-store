...
-- admin_audit_log real schema:
-- id, admin_id, action, target_user(uuid), target_ref(text),
-- amount, balance_before, balance_after, reason, metadata, ip_address, created_at
...
BEGIN
  ...
  THEN
    ...
    IF EXISTS (
      ...
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] idempotent retry tx=% order=%', p_transaction_id, p_order_id;
    ...
    IF EXISTS (SELECT 1 FROM confirmed_transactions WHERE transaction_id=p_transaction_id) THEN
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] tx=% already used for another order — DUPLICATE', p_transaction_id;
  ...
  IF NOT EXISTS (SELECT 1 FROM profiles WHERE id=v_order.user_id) THEN
    RETURN jsonb_build_object('ok',false,'reason','user_not_found'); END IF;
  ...
  IF v_expected_phone<>'' AND v_received_phone<>'' AND v_received_phone<>v_expected_phone THEN
    ...
    IF v_topup.id IS NOT NULL THEN
      UPDATE wallet_topup_requests SET scan_status='manual_review',
        failure_reason='رقم المحول غير مطابق' WHERE id=v_topup.id; END IF;
  ...
  IF ROUND(p_received_amount::numeric,2)<>ROUND(v_order.expected_amount::numeric,2) THEN
    ...
    IF v_topup.id IS NOT NULL THEN
      ...
      WHERE id=v_topup.id AND status NOT IN ('approved','rejected'); END IF;
  ...
  -- ── STEP 8: ATOMIC reserve in payment_transactions (race-safe guard) ───
  IF p_transaction_id IS NOT NULL AND p_transaction_id<>'' THEN
    ...
    EXCEPTION WHEN unique_violation THEN
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] race-condition tx=% collision in payment_transactions', p_transaction_id;
      UPDATE payment_orders SET status='duplicate',
  ...
  -- Secondary record in sms_transaction_receipts
  IF p_transaction_id IS NOT NULL AND p_transaction_id<>'' THEN
    BEGIN
  ...
  -- profiles has only wallet_balance (no credits_balance column)
  UPDATE profiles
  ...
  -- ── STEP 10: Update wallet_topup_request ──────────────────────────────
  IF v_topup.id IS NOT NULL THEN
    BEGIN
  ...
  ELSE
    RAISE NOTICE '[ORDER_UPDATED] no linked topup_request for payment_order=%', p_order_id;
  END IF;

  -- ── Audit logs (correct admin_audit_log schema) ────────────────────────
  -- admin_audit_log: admin_id, action, target_user, target_ref, amount,
