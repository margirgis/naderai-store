...
-- admin_audit_log.admin_id has FK → profiles, so system actions must use NULL
-- Fix confirm_payment_order to use NULL for system/auto actions
...
BEGIN
  ...
  THEN
    ...
    IF EXISTS (SELECT 1 FROM confirmed_transactions WHERE transaction_id=p_transaction_id) THEN
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] tx=% already used for another order', p_transaction_id;
    ...
    IF EXISTS (
      ...
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] tx=% in payment_transactions ledger for different order', p_transaction_id;
      UPDATE payment_orders SET status='duplicate',
  ...
  -- ── STEP 8: ATOMIC reserve in payment_transactions (race-safe) ────────
  IF p_transaction_id IS NOT NULL AND p_transaction_id<>'' THEN
    ...
    EXCEPTION WHEN unique_violation THEN
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] race-condition tx=% — rejected', p_transaction_id;
      UPDATE payment_orders SET status='duplicate',
