...
-- Drop old signature first, then recreate with triple-match
...
BEGIN
  ...
  -- ── STEP 1: Fast-path idempotency + duplicate checks ──────────────────
  ...
  THEN
    ...
    -- Same tx, different order → cross-order duplicate
    IF EXISTS (
      SELECT 1 FROM confirmed_transactions ct
    ...
    -- ── Triple-match check: tx_id + phone + amount all match a previous tx
    IF is_triple_duplicate(p_transaction_id, p_sender_phone, p_received_amount, NULL) THEN
      RAISE NOTICE '[TRIPLE_MATCH_DUPLICATE] tx=% phone=% amount=%',
        p_transaction_id, p_sender_phone, p_received_amount;
      INSERT INTO security_audit_log(event_type,order_id,device_id,details)
      ...
      UPDATE payment_orders SET status='duplicate',
        failure_reason='عملية مكررة: رقم العملية + رقم الهاتف + المبلغ مطابقة لعملية سابقة',
        verification_status='no_match' WHERE id=p_order_id;
      RETURN jsonb_build_object('ok',false,'scan_status','duplicate',
        'reason','triple_match_duplicate',
        'message','رقم العملية '||p_transaction_id||' مع نفس الرقم والمبلغ تم استخدامه مسبقاً');
    END IF;

    -- Race guard via payment_transactions ledger
    IF EXISTS (
  ...
  -- ── DUAL LOOKUP: payment_order_id FK or notes field (legacy) ──────────
  ...
  IF v_expected_phone<>'' AND v_received_phone<>'' AND v_received_phone<>v_expected_phone THEN
    ...
    RETURN jsonb_build_object('ok',false,'scan_status','manual_review',
      'reason','sender_phone_mismatch','expected',v_order.sender_phone,'received',p_sender_phone);
  END IF;
  ...
  IF ROUND(p_received_amount::numeric,2)<>ROUND(v_order.expected_amount::numeric,2) THEN
    ...
    END IF;
    RETURN jsonb_build_object('ok',false,'scan_status','manual_review',
      'reason','amount_mismatch','expected',v_order.expected_amount,'received',p_received_amount);
  END IF;
  ...
  UPDATE profiles SET wallet_balance=COALESCE(wallet_balance,0)+COALESCE(v_order.credits_qty,0) WHERE id=v_order.user_id;
  ...
  VALUES (v_order.user_id,'credit',v_order.credits_qty,v_balance_before,
    v_balance_before+v_order.credits_qty,'شحن رصيد تلقائي - طلب #'||v_order.order_number,
    'PAY-ORDER-'||p_order_id::text);

  -- ── STEP 10: Update wallet_topup_requests ────────────────────────────
  IF v_topup.id IS NOT NULL THEN
    ...
    BEGIN
      INSERT INTO confirmed_transactions(transaction_id,order_id,user_id,sender_phone,sender_name,amount,status,confirmed_at)
      VALUES (COALESCE(p_transaction_id,'AUTO-'||p_order_id::text),v_topup.id,v_order.user_id,
  ...
  -- ── Audit + Notifications ─────────────────────────────────────────────
  INSERT INTO admin_audit_log(admin_id,action,target_user,target_ref,amount,balance_before,balance_after,reason,metadata)
  VALUES (NULL,'auto_confirm_payment_order',v_order.user_id,p_order_id::text,p_received_amount,
    v_balance_before,v_balance_before+v_order.credits_qty,'تأكيد تلقائي - طلب #'||v_order.order_number,
  ...
  VALUES ('payment_confirmed',p_order_id,p_transaction_id,COALESCE('device:'||p_device_id,'system'),
    p_received_amount,jsonb_build_object('credits_added',v_order.credits_qty,
      'user_id',v_order.user_id,'topup_id',v_topup.id));
  ...
  RETURN jsonb_build_object(
    'ok',true,'scan_status','confirmed','order_id',p_order_id,
    'order_number',v_order.order_number,'credits_added',v_order.credits_qty,
    'transaction_id',p_transaction_id,'topup_id',v_topup.id
  );
END;
$$;
