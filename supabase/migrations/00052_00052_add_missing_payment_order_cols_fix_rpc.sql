...
-- ═══════════════════════════════════════════════════════════════════════════
-- Migration 00052: Add missing columns to payment_orders, fix confirm_payment_order
--
-- FACTS DISCOVERED:
-- • payment_orders has NO: verified_at, transaction_id, failure_reason
-- • confirmed_transactions.order_id FK → wallet_topup_requests (not payment_orders)
-- • confirmed_transactions has NO device_id column
-- • wallet_topup_requests has: payment_order_id, transaction_id, failure_reason
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Unique index on transaction_id for payment_orders (prevent same tx used twice)
CREATE UNIQUE INDEX IF NOT EXISTS payment_orders_tx_id_unique
  ON payment_orders(transaction_id)
  WHERE transaction_id IS NOT NULL;

-- ── Replace confirm_payment_order with schema-correct version ──────────────
...
BEGIN
  ...
  -- ── STEP 1: Fast-path idempotency (no row lock yet) ──────────────
  ...
  THEN
    ...
    -- confirmed_transactions.order_id FK → wallet_topup_requests
    -- We look it up via the topup linked to this payment_order
    IF EXISTS (
    ...
    -- Same tx but used for a DIFFERENT order → fraud/replay
    IF EXISTS (
      SELECT 1 FROM confirmed_transactions
      WHERE transaction_id = p_transaction_id
    ) THEN
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] tx=% already in confirmed_transactions for other order',
        p_transaction_id;
    ...
    -- Also check payment_transactions ledger (faster index)
    IF EXISTS (SELECT 1 FROM payment_transactions WHERE transaction_id = p_transaction_id AND status = 'accepted') THEN
      IF NOT EXISTS (SELECT 1 FROM payment_transactions WHERE transaction_id = p_transaction_id AND order_id = p_order_id) THEN
        RAISE NOTICE '[TRANSACTION_ALREADY_USED] tx=% in payment_transactions for other order', p_transaction_id;
        UPDATE payment_orders
          SET status         = 'duplicate',
              failure_reason = 'رقم العملية مستخدم في طلب آخر (ledger): ' || p_transaction_id
        WHERE id = p_order_id;
        RETURN jsonb_build_object(
  ...
  IF p_sms_timestamp IS NOT NULL THEN
    IF p_sms_timestamp < (v_order.created_at - SMS_WINDOW) THEN
      ...
      UPDATE payment_orders SET status = 'failed',
        failure_reason = 'SMS أقدم من وقت إنشاء الطلب' WHERE id = p_order_id;
      RETURN jsonb_build_object('ok', false, 'scan_status', 'failed',
        'reason', 'sms_too_old',
    ...
    IF v_topup.id IS NOT NULL THEN
      UPDATE wallet_topup_requests
        SET scan_status = 'manual_review',
            failure_reason = 'رقم المحول غير مطابق: متوقع ' || v_order.sender_phone
                             || ' تم استلام ' || p_sender_phone
      WHERE id = v_topup.id;
    END IF;
    RETURN jsonb_build_object(
  ...
  IF ROUND(p_received_amount::numeric, 2) <> ROUND(v_order.expected_amount::numeric, 2) THEN
    ...
    END IF;
    RETURN jsonb_build_object(
      'ok', false, 'scan_status', 'manual_review', 'reason', 'amount_mismatch',
      'expected', v_order.expected_amount, 'received', p_received_amount
    );
  END IF;

  -- ── STEP 8: ATOMIC reserve in payment_transactions (UNIQUE guard) ──
  -- This is the PRIMARY duplicate prevention: concurrent calls both try
  -- this INSERT; only the first succeeds due to UNIQUE(transaction_id).
  IF p_transaction_id IS NOT NULL AND p_transaction_id <> '' THEN
    ...
    EXCEPTION WHEN unique_violation THEN
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] race-condition tx=% payment_transactions collision',
        p_transaction_id;
      UPDATE payment_orders
        SET status         = 'duplicate',
  ...
  -- Also insert in sms_transaction_receipts (legacy table)
  IF p_transaction_id IS NOT NULL AND p_transaction_id <> '' THEN
  ...
  -- ── STEP 10: Update wallet_topup_request (by payment_order_id) ─────
  IF v_topup.id IS NOT NULL THEN
    -- Insert into confirmed_transactions (FK → wallet_topup_requests.id)
    BEGIN
      INSERT INTO confirmed_transactions(
        ...
        v_topup.id,           -- ← correct FK: wallet_topup_requests.id
        v_order.user_id,
    ...
    RAISE NOTICE '[ORDER_UPDATED] topup_id=% status=approved', v_topup.id;
  END IF;
  ...
  VALUES (
    ...
    jsonb_build_object(
      'transaction_id', p_transaction_id, 'credits_added', v_order.credits_qty,
  ...
  VALUES (
    ...
    p_received_amount,
    jsonb_build_object(
      'credits_added', v_order.credits_qty, 'user_id', v_order.user_id,
