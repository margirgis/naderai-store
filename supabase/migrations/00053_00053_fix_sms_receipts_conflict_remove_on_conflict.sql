...
-- Add UNIQUE constraint to sms_transaction_receipts.transaction_id if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = 'sms_transaction_receipts'::regclass
      AND contype = 'u'
      AND conname LIKE '%transaction_id%'
  ) THEN
    ALTER TABLE sms_transaction_receipts
      ADD CONSTRAINT str_txn_id_unique UNIQUE (transaction_id);
  END IF;
END $$;

-- Re-create confirm_payment_order: replace ON CONFLICT(transaction_id) on
-- sms_transaction_receipts with a plain INSERT wrapped in EXCEPTION handler
...
BEGIN
  ...
  THEN
    -- Same tx, same topup order → idempotent retry
    IF EXISTS (
    ...
    IF EXISTS (SELECT 1 FROM confirmed_transactions WHERE transaction_id = p_transaction_id) THEN
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] tx=% in confirmed_transactions for another order', p_transaction_id;
      ...
      UPDATE payment_orders SET status = 'duplicate', failure_reason = 'رقم العملية مستخدم في طلب آخر: ' || p_transaction_id, verification_status = 'no_match' WHERE id = p_order_id;
    ...
    -- Check payment_transactions ledger
    IF EXISTS (SELECT 1 FROM payment_transactions WHERE transaction_id = p_transaction_id AND status = 'accepted' AND order_id <> p_order_id) THEN
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] tx=% in payment_transactions for different order', p_transaction_id;
      UPDATE payment_orders SET status = 'duplicate', failure_reason = 'رقم العملية مستخدم (ledger): ' || p_transaction_id WHERE id = p_order_id;
      RETURN jsonb_build_object('ok', false, 'scan_status', 'duplicate', 'reason', 'duplicate_transaction_id_ledger');
    END IF;
  END IF;
  ...
  IF v_order.expires_at <= now() AND v_order.status NOT IN ('reopened') THEN
    ...
    IF v_topup.id IS NOT NULL THEN UPDATE wallet_topup_requests SET status = 'expired' WHERE id = v_topup.id; END IF;
  ...
  IF v_expected_phone <> '' AND v_received_phone <> '' AND v_received_phone <> v_expected_phone THEN
    ...
    UPDATE payment_orders SET status='manual_review',verification_status='no_match',failure_reason='رقم المحول غير مطابق' WHERE id=p_order_id;
    IF v_topup.id IS NOT NULL THEN
      UPDATE wallet_topup_requests SET scan_status='manual_review',failure_reason='رقم المحول غير مطابق' WHERE id=v_topup.id;
    END IF;
    RETURN jsonb_build_object('ok',false,'scan_status','manual_review','reason','sender_phone_mismatch','expected',v_order.sender_phone,'received',p_sender_phone);
  END IF;
  ...
  IF ROUND(p_received_amount::numeric,2) <> ROUND(v_order.expected_amount::numeric,2) THEN
    ...
    UPDATE payment_orders SET status='amount_mismatch',failure_reason='مبلغ غير مطابق: مطلوب '||v_order.expected_amount||' تم استلام '||p_received_amount WHERE id=p_order_id;
    IF v_topup.id IS NOT NULL THEN
      UPDATE wallet_topup_requests SET scan_status='manual_review',failure_reason='المبلغ غير مطابق: مطلوب '||v_order.expected_amount||' تم استلام '||p_received_amount WHERE id=v_topup.id AND status NOT IN ('approved','rejected');
    END IF;
    RETURN jsonb_build_object('ok',false,'scan_status','manual_review','reason','amount_mismatch','expected',v_order.expected_amount,'received',p_received_amount);
  END IF;

  -- ── STEP 8: ATOMIC reserve in payment_transactions (primary guard) ───
  IF p_transaction_id IS NOT NULL AND p_transaction_id <> '' THEN
    ...
    EXCEPTION WHEN unique_violation THEN
      RAISE NOTICE '[TRANSACTION_ALREADY_USED] race-condition tx=% payment_transactions collision', p_transaction_id;
      UPDATE payment_orders SET status='duplicate',failure_reason='رقم العملية وُجد مسبقاً (race): '||p_transaction_id WHERE id=p_order_id;
      RETURN jsonb_build_object('ok',false,'scan_status','duplicate','reason','race_condition_duplicate');
    END;
  END IF;

  -- Write to sms_transaction_receipts (no unique constraint, use plain insert)
  IF p_transaction_id IS NOT NULL AND p_transaction_id <> '' THEN
    BEGIN
  ...
  UPDATE payment_orders SET status='confirmed', verified_at=now(), confirmed_at=now(),
  ...
  VALUES (v_order.user_id,'credit',v_order.credits_qty,v_balance_before,
    ...
    'شحن رصيد تلقائي - طلب #'||v_order.order_number,'PAY-ORDER-'||p_order_id::text);

  -- ── STEP 10: Update topup_request + insert confirmed_transactions ────
  IF v_topup.id IS NOT NULL THEN
    BEGIN
    ...
    UPDATE wallet_topup_requests
      SET status='approved', scan_status='approved', verification_status='completed',
  ...
  VALUES ('00000000-0000-0000-0000-000000000000'::uuid,
    ...
    jsonb_build_object('transaction_id',p_transaction_id,'credits_added',v_order.credits_qty,
      'amount',p_received_amount,'user_id',v_order.user_id,'device_id',p_device_id,'topup_id',v_topup.id),
    now());
  ...
  PERFORM create_admin_notification(
    ...
    p_event_type:='payment_confirmed',p_reference_id:=p_order_id::text,p_device_id:=p_device_id);
