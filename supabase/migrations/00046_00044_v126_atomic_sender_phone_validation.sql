-- ═══════════════════════════════════════════════════════════════════════════
-- Migration 00044 (v1.1.26): Atomic sender phone validation + transaction lock
-- 1. confirm_payment_order: reject if sender_phone does not match order
-- 2. atomic_confirm_topup: reject if sender_phone does not match request
-- 3. Reserve transaction_id in sms_transaction_receipts before credit
-- 4. Helper: normalize_egyptian_phone already exists
-- ═══════════════════════════════════════════════════════════════════════════

-- ── 1. Replace confirm_payment_order with sender phone validation ────────
...
BEGIN
  ...
  -- ── 6. Sender phone validation (v1.1.26) ─────────────────────────────────
  ...
  IF v_expected_phone <> '' AND v_received_phone <> v_expected_phone THEN
    INSERT INTO security_audit_log(event_type, user_id, order_id, device_id, details)
    ...
    UPDATE payment_orders SET status = 'manual_review' WHERE id = p_order_id;
    UPDATE wallet_topup_requests
    SET scan_status = 'manual_review',
        failure_reason = 'رقم المحول غير مطابق: متوقع ' || v_order.sender_phone || ' تم استلام ' || p_sender_phone
  ...
  -- ── 8. Reserve transaction_id in audit ledger before credit ────────────────
  IF p_transaction_id IS NOT NULL AND p_transaction_id <> '' THEN
    BEGIN
      INSERT INTO sms_transaction_receipts(
        ...
        receiver_wallet, sms_body, device_id, payment_order_id, status
      ) VALUES (
        p_transaction_id, p_sender_phone, p_sender_name, p_received_amount,
        NULL, p_sms_body, p_device_id, p_order_id, 'accepted'
      );
    EXCEPTION WHEN unique_violation THEN
      UPDATE payment_orders SET status = 'duplicate' WHERE id = p_order_id;
      RETURN jsonb_build_object('ok', false, 'scan_status', 'duplicate',
        'reason', 'transaction_id_already_reserved');
    END;
  END IF;

  -- ── 9. Final idempotency guard: already in confirmed_transactions ────────
  ...
  -- ── 10. Atomic insert into confirmed_transactions ────────────────────────
  BEGIN
  ...
  -- ── 11. Confirm order & credit wallet ────────────────────────────────────
...
-- ── 2. Replace atomic_confirm_topup with sender phone validation ───────────
...
BEGIN
  ...
  -- ── Sender phone validation (v1.1.26) ────────────────────────────────────
  ...
  IF norm_expected <> '' AND norm_received <> norm_expected THEN
    INSERT INTO security_audit_log(event_type, order_id, device_id, details)
    ...
    RETURN jsonb_build_object(
      'ok', false, 'reason', 'sender_phone_mismatch',
  ...
  -- ── Reserve transaction_id in audit ledger ───────────────────────────────
  IF p_transaction_id IS NOT NULL AND p_transaction_id <> '' THEN
    BEGIN
      INSERT INTO sms_transaction_receipts(
        transaction_id, sender_phone, sender_name, amount,
  ...
  -- ── Duplicate transaction_id check in confirmed_transactions ─────────────
...
-- ── 3. Ensure RLS policy for admin reads on sms_transaction_receipts ───────
