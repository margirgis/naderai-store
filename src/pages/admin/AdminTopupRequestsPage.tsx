        ...
        <div className="flex items-center gap-3">
          ...
          <div className="space-y-0.5 flex-1 min-w-0">
            <h1 className="text-xl font-bold text-foreground flex items-center gap-2">
              ...
              {scanning > 0 && <Badge variant="secondary" className="text-xs gap-1"><ScanLine className="w-3 h-3" /> {scanning} قيد الفحص</Badge>}
            </h1>
                    ...
                    <div key={r.id} className="p-4 space-y-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          ...
                          {r.scan_status && r.scan_status !== r.status && (
                            <span className="text-xs text-muted-foreground">{SCAN_LABELS[r.scan_status] ?? r.scan_status}</span>
                          )}
                        </div>
                      </div>
                      ...
                      {r.assigned_device_id && (
                        <div className="p-2.5 rounded-lg bg-primary/5 border border-primary/10 text-xs">
                          <div className="flex items-center gap-1.5 text-muted-foreground">
                            ...
                            <span>جهاز الفحص: {r.assigned_device_id.slice(0, 12)}...</span>
                          </div>
                          {r.scanning_started_at && (
                            <p className="text-muted-foreground mt-1">
                              بدأ الفحص: {new Date(r.scanning_started_at).toLocaleString('ar-SA')}
                            </p>
                          )}
                        </div>
                      )}
                      ...
                      {failReason && (r.status === 'pending' || r.scan_status === 'manual_review') && (
                      ...
                      {(r.status === 'pending' || r.scan_status === 'manual_review') && (
                        <div className="flex items-center gap-2">
