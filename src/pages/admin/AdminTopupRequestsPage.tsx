...
const STATUS_LABELS: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline' }> = {
  ...
  amount_mismatch: { label: 'مبلغ غير مطابق', variant: 'destructive' },
  duplicate:       { label: 'مكرر',           variant: 'outline' },
            ...
            {loading ? (
              ...
              <div className="divide-y divide-border">
                {requests.map((r) => {
                  ...
                  // نفضل عرض scan_status كحالة فعالة عند وجوده، ما لم يكن الطلب مكتمل/مرفوض
                  const effectiveStatus = (r.status !== 'approved' && r.status !== 'rejected' && scanStatus)
                    ? scanStatus
                    : r.status;
                  const statusCfg = STATUS_LABELS[effectiveStatus] ?? scanCfg ?? { label: effectiveStatus, variant: 'outline' as const };
