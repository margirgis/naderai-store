...
|---|---|
| Region | Singapore |
| Third-party domain | `app-dpgpkghtekg1-api-e94GlB86xpEa.gateway.appmedo.com` |
| Method | `POST` |
...
| API Plugin endpoint | `POST https://app-dpgpkghtekg1-api-e94GlB86xpEa.gateway.appmedo.com/image-to-video/kling-3.0-turbo` |
...
while (true) {
  const queryResponse = await fetch(
    `https://app-dpgpkghtekg1-api-GaDwzVOqmRNY.gateway.appmedo.com/tasks?task_ids=${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
